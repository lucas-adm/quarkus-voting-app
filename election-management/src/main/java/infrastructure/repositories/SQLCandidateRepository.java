...
public class SQLCandidateRepository implements CandidateRepository {
    ...
    public List<Candidate> find(CandidateQuery query) {
        ...
        // SELECT * FROM candidate WHERE id IN (:idList) OR(LOWER(family_name) LIKE LOWER(CONCAT(:name, '%')) OR LOWER(given_name) LIKE LOWER(CONCAT(:name, '%')));
    ...
    private Predicate[] conditions(CandidateQuery query, CriteriaBuilder cb, Root<infrastructure.repositories.entities.Candidate> root) {
        return Stream.of(query.ids().map(id -> cb.in(root.get("id")).value(id)),
                ...
                .toArray(Predicate[]::new); //Hibernate sempre espera um array
    }
